...
// How long ago an order was entered — "just now" through to "2 years ago".
// Steps up a unit at a time so the label stays short at any age.
              ...
              {data.all.map((o) => (
                <div key={o.id} className={'mini-table-row' + (o.isNew24 ? ' row-new24' : '')} onClick={() => router.push(`/orders/${o.id}`)}>
                  <div className="mono" style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--heading)' }}>
                    {o.order_no}
                    {o.created_at && (
                      <div className={o.isNew24 ? 'new24-tag' : 'since-tag'}>
                        {o.isNew24 ? '🆕 ' : ''}{sinceLabel(o.created_at)}
                      </div>
                    )}
